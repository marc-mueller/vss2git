...
namespace Hpdi.Vss2Git
{
    ...
    {
        private static UserFeedbackService current = new UserFeedbackService();
        ...
        public static UserFeedbackService Current
        {
            get
            {
                return current;
        ...
        public FeedbackResult Show(string message, string title, FeedbackOptions options, FeedbackIcon icon)
        {
            if(HandleUserFeedback != null)
            {
                var args = new UserFeedbackEventArgs(message, title, options, icon);
                HandleUserFeedback(this, args);
                return args.FeedbackResult;
            }
            return FeedbackResult.None;
        }

        public event UserFeedbackHandler HandleUserFeedback;

    }

    public delegate void UserFeedbackHandler(object sender, UserFeedbackEventArgs args); 

    public class UserFeedbackEventArgs : EventArgs
    {
        public UserFeedbackEventArgs(string message, string title, FeedbackOptions options, FeedbackIcon icon)
        {
            Message = message;
            Title = title;
            Options = options;
        ...
        public FeedbackOptions Options { get; private set; }
        public string Title { get; private set; }

        public FeedbackResult FeedbackResult { get; set; }
        public FeedbackIcon Icon { get; private set; }
    }
